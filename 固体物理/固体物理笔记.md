...
- $\omega=2 \sqrt{\frac{eta}{m}}\left|\sin \frac{1}{2} a q\right|$
    + $eta$为恢复力常数
    + m为粒子质量
...
$$\left\\\{\begin{aligned} M \frac{\mathrm{d}^{2} u_{2 n}}{\mathrm{d} t^{2}} &=eta\left(u_{2 n+1}+u_{2 n-1}-2 u_{n}\right) \\\\ m \frac{\mathrm{d}^{2} u_{2 n+1}}{d t^{2}} &=eta\left(u_{2 n}+u_{2 n+2}-2 u_{2 n+1}\right) \end{aligned}\right.$$

色散关系：
$$
\omega_{ \pm}^{2}=\frac{eta}{m M}\left[(m+M) \pm\left(m^{2}+M^{2}+2 m M \cos 2 q a\right)^{\frac{1}{2}}\right]
$$
...
$$\overline{\varepsilon}_{i}=\frac{\hbar \omega_{i}}{2}+\frac{\hbar \omega_{i}}{e^{\hbar \omega_{i} / k_{B} T}-1}$$
...
$$\overline{E}=\sum_{i=1}^{3 N} \overline{\varepsilon}_{i}=\sum_{i=1}^{3 N} \frac{\hbar \omega_{i}}{e^{\frac{\hbar \omega_{i}}{k_{B} T}}-1}=\int_{0}^{\omega_{D}} g(\omega) \frac{\hbar \omega}{e^{\frac{\hbar \omega}{k_{B} T}}-1} \mathrm{d} \omega$$
...
$$
\overline{\varepsilon}_{j}=\left(\overline{n}_{j}+\frac{1}{2}\right) \hbar \omega_{j} \qquad \overline{n}_{j}=\frac{1}{\exp \left(\frac{\hbar \omega_{1}}{k_{s} T}\right)-1}
$$
...
光学支q=0时的频率
$$
\omega_{\mathrm{TO}}^{2}=2 eta \frac{M_{-}+M_{+}}{M_{-} M_{+}}=\frac{2 eta}{\mu}
$$

运动：
$$
\left\{\begin{aligned} u_{0+} &=\frac{e^{*} E_{0}}{M_{+}\left[2 eta\left(\frac{M_{-}+M_{+}}{M_{+} M_{-}}\right)-\omega^{2}\right]}=\frac{e^{*} E_{0}}{M_{+}\left(\omega_{\mathrm{TO}}^{2}-\omega^{2}\right)} \\ u_{o-}=& \frac{-e^{*} E_{0}}{M_{-}\left(\omega_{\mathrm{TO}}^{2}-\omega^{2}\right)} \end{aligned}\right.
$$
...
晶格热传导：声子扩散运动的结果
$$\kappa=\frac13C_V\lambda\overline{v}_s$$

声子碰撞：服从能量动量守恒
$$
\begin{aligned}
\hbar\omega_1+\hbar\omega_2=\hbar\omega_3\\\\
\hbar\vec{q}_1+\hbar\vec{q}_2=\hbar\vec{q}_3\quad\textrm{正常过程}\\\\
\hbar\vec{q}_1+\hbar\vec{q}_2=\hbar\vec{q}_3+\hbar\vec{G}\quad{倒逆过程}
\end{aligned}
$$
...
$$
\begin{array}{l}{\vec{k}=\vec{k}_{0}+\vec{q}} \\ {\Omega=\Omega_{0} \pm \omega(q)}\end{array}
$$
...
$$
\left\\\{\begin{aligned} E_{+} & \approx E_{k^{\prime}}^{(0)}+\frac{\left|U_{n}\right|^{2}}{E_{k^{\prime}}^{(0)}-E_{k}^{(0)}} \\ E_{-} & \approx E_{k}^{(0)}-\frac{\left|U_{n}\right|^{2}}{E_{k^{\prime}}^{(0)}-E_{k}^{(0)}} \end{aligned}\right.
$$
...
- 电子的波函数是单原子核情况的线性叠加
    ...
    + $$\sum_{m} a_{m}\left[\varepsilon_{i}+U({r})-V\left({r}-{R}_{m}\right)\right] \varphi_{i}\left({r}-{R}_{m}\right)=E \sum_{m} a_{m} \varphi_{i}\left(r-{R}_{m}\right)$$
- 原子间距大于轨道半径
    + $$\int\varphi_{i}^*(r-R_n)\varphi_i(r-R_m)\d r=\delta_{nm}$$

目标：求解$E$
$$J(R_n-R_m):=-\left<\varphi_i(r-(R_n-R_m))\right|U(r)-V(r)\left|\varphi_i(r)\right>$$
$$-\sum_ma_mJ(R_n-R_m)=(E-\varepsilon_i)a_n$$
解出$a_m$:

$$a_m=Ce^{ik\cdot R_m}$$
带回方程：

$$E-\varepsilon_i=-\sum J(R_{n}-{R}_{m}) e^{-i k \cdot(R_{n}-R_{m})}$$


对于确定的${k}$，电子运动的波函数为

$$
\begin{aligned} E-\varepsilon_{i} &=-\sum_{m} J\left(R_{n}-R_{m}\right) e^{-i k\left(R_{n}-R_{n}\right)} \\ &=-\sum_{s} J\left(R_{s}\right) e^{-i k \cdot R_{s}} \qquad R_{s}=R_{n}-\boldsymbol{R}_{m} \end{aligned}
$$

能量本征值

$$E(k)=\varepsilon_{j}-\sum_{s} J{R}_{s} e^{-i k \cdot R_{s}}$$

一般只保留近邻项：
$$E({k})=\varepsilon_i-J_0-\sum_{{R}_s=\textrm{近邻}}J({R_s})\exp(-i{k}\cdot{R}_s)$$
...
单位体积能态密度：$$N(E)=\frac1V\fd{Z}{E}$$
$\d Z$为能量在 $E-E+dE$ 两等能面间的能态数(考虑了电子自旋)。

$$N(E)=\frac1V\fd{Z}{E}=\frac1{4\pi^3}\iint_{E=const}\frac{\d S}{\left|\nabla_kE(k)\right|}$$
